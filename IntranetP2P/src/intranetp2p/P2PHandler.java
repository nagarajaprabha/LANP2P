...
public class P2PHandler {
	...
	public P2PHandler() {
		...
		server.startServer();
		/**
		 * ClientSocket Responsibilities are 1. Listen to the ClientRequests and
		 * Sends the Responses
		 */
		...
		try {
			...
			while (true) {
				...
				if (br.readLine().contains("isFileAvailable")) {
					 ...
					 */
					out.write(mgr.getFileByName(br.readLine().substring(
							"getFile".length())));

				} else if (br.readLine().contains("partFile")) {
					// processing the response
					/**
					 * First Line : getFile+"FileName" Second Line : int offset
					 * Third Line : int length
					 */
					out.write(mgr.getFile(br.readLine().substring(
							"getFile".length()), Integer
							.parseInt(br.readLine()), Integer.parseInt(br
							.readLine())));
	...
	public void notifyAllPeers(String fileurl) throws UnknownHostException {
		...
		try {
			...
			while ((ipAddress = br.readLine()) != null) {
				 ...
				 * FirstLine : isFileAvailable SecondLine : FileName
				 */
				clntSocket[i].getOutputStream().write(
						("isFileURLAvailable\n" + fileurl).getBytes());
				i++;
